...
int feenox_assign_single(assignment_t *assignment, unsigned int row, unsigned int col) {
  ...
  if (assignment->variable != NULL) {
    ...
    current = feenox_lowlevel_vector_get_ptr(feenox_value_ptr(assignment->vector), row);
    if (assignment->vector->initial_static != NULL) {
      initial_static = feenox_lowlevel_vector_get_ptr(assignment->vector->initial_static, row);
    }
    if (assignment->vector->initial_transient != NULL) {
      initial_transient = feenox_lowlevel_vector_get_ptr(assignment->vector->initial_transient, row);
    } 
    ...
    current = feenox_lowlevel_matrix_get_ptr(feenox_value_ptr(assignment->matrix), row, col);
    if (assignment->matrix->initial_static != NULL) {
      initial_static = feenox_lowlevel_matrix_get_ptr(assignment->matrix->initial_static, row, col);
    }
    if (assignment->matrix->initial_transient != NULL) {
      initial_transient = feenox_lowlevel_matrix_get_ptr(assignment->matrix->initial_transient, row, col);
    }
