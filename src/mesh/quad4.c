...
*/     
  ...
  feenox_call(feenox_mesh_gauss_init_quad4(element_type, &element_type->gauss[integration_full]));
  ...
  for (j = 0; j < element_type->nodes; j++) {
    ...
    for (v = 0; v < 4; v++) {
      feenox_lowlevel_matrix_set(element_type->gauss[integration_full].extrap, j, v, feenox_mesh_quad4_h(v, r));
    }
  }
  
  
  // reduced integration: 1x1
  feenox_call(feenox_mesh_gauss_init_quad1(element_type, &element_type->gauss[integration_reduced]));
