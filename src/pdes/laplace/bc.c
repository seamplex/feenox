/*------------ -------------- -------- --- ----- ---   --       -            -
 *  feenox's routines for Laplace equation: boundary conditions
 *
...
int feenox_problem_bc_set_laplace_phi(bc_data_t *bc_data, size_t node_index) {
...
int feenox_problem_bc_set_laplace_derivative(element_t *element, bc_data_t *bc_data, unsigned int v) {
  ...
  if (bc_data->nonlinear) {
    ...
    gsl_blas_dgemm(CblasTrans, CblasNoTrans, +w*dderivativedphi, element->H[v], element->H[v], 1.0, feenox.pde.Jbi);
  }
