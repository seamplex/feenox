...
create a mesh `nafems-le10.msh` (check out the [tensile test tutorial]
for details). Then, we can create an input file for FeenoX (using
[editors with syntax highlighting] for example) as follows:
...
the same directory, say in the [`examples`] directory of the repository,
then we call first Gmsh and then FeenoX from the terminal to solve the
benchmark problem:
