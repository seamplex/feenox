...
\lhead{$title$}
$endif$
\lfoot{\scriptsize{$if(date)$ $date$ $endif$ / $if(version)$ \texttt{$version$} $endif$ / $if(version)$ \texttt{$hash$} $endif$}}
...
\fancyhf{} % clear all header and footer fields
$if(hash)$
\cfoot{\scriptsize{$if(date)$ $date$ $endif$ / $if(hash)$ \texttt{$hash$} $endif$}}
$endif$
...
$endif$
\input{$rootdir$/syntax-feenox}
\input{$rootdir$/syntax}
$for(header-includes)$
$header-includes$
$endfor$
\ifLuaTeX
...
\thispagestyle{plain}
\begin{center}\huge\bfseries\@title\unskip\strut\par\end{center}
}
