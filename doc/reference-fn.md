...
`atan2(y,x)` $= \displaystyle \arctan(y/x)$
...
`ceil(x)` $= \displaystyle \lceil x \rceil$
...
difference	0.000151128	[seconds]
$
~~~
...
`d_dt(x)` $= \displaystyle \frac{x(t) - x(t-\Delta t)}{\Delta t} \approx \frac{d}{dt} \Big( x (t) \Big)$
...
`deadband(x, a)` $= \displaystyle \begin{cases} 0 & \text{if $| x | \leq a$} \\ x + a & \text{if $x < a$} \\ x - a & \text{if $x > a$} \end{cases}$
...
`equal(a, b, [eps])` $= \displaystyle \begin{cases} 1 & \text{if $a = b$} \\ 0 & \text{if $a \neq b$} \end{cases}$
...
`expint1(x)` $= \displaystyle \text{Re} \left[ \int_1^{\infty}\! \frac{\exp(-xt)}{t} \, dt \right]$
...
`expint2(x)` $= \displaystyle \text{Re} \left[ \int_1^{\infty}\! \frac{\exp(-xt)}{t^2} \, dt \right]$
...
`expint3(x)` $= \displaystyle \text{Re} \left[ \int_1^{\infty}\! \frac{\exp(-xt)}{t^3} \, dt \right]$
...
`expintn(n,x)` $= \displaystyle \text{Re} \left[ \int_1^{\infty}\! \frac{\exp(-xt)}{t^n} \, dt \right]$
...
`floor(x)` $= \displaystyle \lfloor x \rfloor$
...
`heaviside(x, [delta])` $= \displaystyle \begin{cases} 0 & \text{if $x < 0$} \\ x / \delta & \text{if $0 < x < \delta$} \\ 1 & \text{if $x > \delta$} \end{cases}$
...
`if(a, [b], [c], [eps])` $= \displaystyle \begin{cases} b & \text{if $|a|<\epsilon$} \\ c & \text{otherwise} \end{cases}$
...
`integral_dt(x)` $= \displaystyle z^{-1}\left[ \int_0^{t-\Delta t} x(t') \, dt' \right] +  \frac{x(t) + x(t-\Delta t)}{2} \, \Delta t \approx \int_0^{t} x(t') \, dt'$
...
`integral_euler_dt(x)` $= \displaystyle z^{-1}\left[ \int_0^{t-\Delta t} x(t') \, dt' \right] +   x(t) \, \Delta t \approx \int_0^{t} x(t') \, dt'$
...
`is_even(x)` $= \displaystyle  \begin{cases}1 &\text{if $x$ is even} \\ 0 &\text{if $x$ is odd} \end{cases}$
...
`is_in_interval(x, a, b)` $= \displaystyle \begin{cases} 1 & \text{if $a \leq x < b$} \\ 0 & \text{otherwise} \end{cases}$
...
`is_odd(x)` $= \displaystyle  \begin{cases}1 &\text{if $x$ is odd} \\ 0 &\text{if $x$ is even} \end{cases}$
...
`lag(x, tau)` $= \displaystyle x(t) - \Big[ x(t) - y(t-\Delta t) \Big] \cdot \exp\left(-\frac{\Delta t}{\tau}\right)$
...
`lag_bilinear(x, tau)` $= \displaystyle x(t-\Delta t) \cdot \left[ 1 - \frac{\Delta t}{2\tau} \right] + \left[ \frac{x(t) + x(t - \Delta t)}{1 + \frac{\Delta t}{2\tau}}\right] \cdot \frac{\Delta t}{2\tau}$
...
`lag_euler(x, tau)` $= \displaystyle x(t-\Delta t) + \Big[ x(t) - x(t - \Delta t) \Big] \cdot \frac{\Delta t}{\tau}$
...
`last(x,[p])` $= \displaystyle z^{-1}\left[ x \right] = x(t-\Delta t)$
...
`limit(x, a, b)` $= \displaystyle \begin{cases} a & \text{if $x < a$} \\ x & \text{if $a \leq x \leq b$} \\ b & \text{if $x > b$} \end{cases}$
...
`limit_dt(x, a, b)` $= \displaystyle \begin{cases} x(t) & \text{if $a \leq dx/dt \leq b$} \\ x(t-\Delta t) + a \cdot \Delta t & \text{if $dx/dt < a$} \\ x(t-\Delta t) + b \cdot \Delta t & \text{if $dx/dt > b$} \end{cases}$
...
`mark_max(x1, x2, [...], [x10])` $= \displaystyle  i / \max \Big (x_1, x_2, \dots, x_{10} \Big) = x_i$
...
`mark_max(x1, x2, [...], [x10])` $= \displaystyle  i / \min \Big (x_1, x_2, \dots, x_{10} \Big) = x_i$
...
`max(x1, x2, [...], [x10])` $= \displaystyle  \max \Big (x_1, x_2, \dots, x_{10} \Big)$
...
`min(x1, x2, [...], [x10])` $= \displaystyle  \min \Big (x_1, x_2, \dots, x_{10} \Big)$
...
`mod(a, b)` $= \displaystyle a - \left\lfloor \frac{a}{b} \right\rfloor \cdot b$
...
`not(x, [eps])` $= \displaystyle  \begin{cases}1 &\text{if $|x| < \epsilon$} \\ 0 &\text{otherwise} \end{cases}$
...
`random(x1, x2, [s])` $= \displaystyle  x_1 + r \cdot (x_2-x_1) \quad \quad 0 \leq r < 1$
...
`round(x)` $= \displaystyle \begin{cases} \lceil x \rceil & \text{if $\lceil x \rceil - x < 0.5$} \\ \lceil x \rceil & \text{if $\lceil x \rceil - x = 0.5 \wedge x > 0$} \\ \lfloor x \rfloor & \text{if $x-\lfloor x \rfloor < 0.5$} \\ \lfloor x \rfloor & \text{if $x-\lfloor x \rfloor = 0.5 \wedge x < 0$} \end{cases}$
...
`sawtooth_wave(x)` $= \displaystyle x - \lfloor x \rfloor$
...
`sgn(x, [eps])` $= \displaystyle  \begin{cases}-1 &\text{if $x \le -\epsilon$} \\ 0 &\text{if $|x| < \epsilon$} \\ +1 &\text{if $x \ge +\epsilon$} \end{cases}$
...
`square_wave(x)` $= \displaystyle \begin{cases} 1 & \text{if $x - \lfloor x \rfloor < 0.5$} \\ 0 & \text{otherwise} \end{cases}$
...
`threshold_max(x, a, [b])` $= \displaystyle \begin{cases} 1 & \text{if $x > a$} \\ 0 & \text{if $x < a-b$} \\ \text{last value of $y$} & \text{otherwise} \end{cases}$
...
`threshold_min(x, a, [b])` $= \displaystyle \begin{cases} 1 & \text{if $x < a$} \\ 0 & \text{if $x > a+b$} \\ \text{last value of $y$} & \text{otherwise} \end{cases}$
...
`triangular_wave(x)` $= \displaystyle \begin{cases} 2 (x - \lfloor x \rfloor) & \text{if $x - \lfloor x \rfloor < 0.5$} \\ 2 [1-(x - \lfloor x \rfloor)] & \text{otherwise} \end{cases}$
